...
cartella_destinazione = f"{config.get('DATI2D', 'cartella')}/radar_etp"

# %%
sovrascrivi = False
livelli_dBz = [18, 35, 45]
...
lista_tempi = pd.date_range('2026-07-21 19:30:00', adesso_0_UTC + pd.Timedelta(hours=1), freq='5min')
...
for adesso_0_UTC in lista_tempi:
    ...
    for dBZ in livelli_dBz:
    
        for t in pd.date_range(adesso_0_UTC - pd.Timedelta(hours=1), adesso_0_UTC, freq='5min')[::-1]:
            percorso = f"/mnt/ARC_STORICO/RADAR/ARCHIVIO_RADAR_ETP/{t.strftime(format='%Y/%m/%d')}/ETP{t.strftime(format='%Y%m%d%H%M')}{dBZ}.tif"
        ...
        cartella_file = f"{cartella_destinazione}/{dBZ}/{t.strftime(format='%Y/%m/%d')}"
        os.makedirs(cartella_file, exist_ok=True)
        nome_file_webp = f"radar_etp_{t.strftime(format='%Y-%m-%d_%H%M')}.webp"
        ...
        # livelli = np.array([-1.5, -0.5, 1000, 2000, 3000, 4000, 5000, 6000, 7000, 8000, 9000, 10000, 11000, 12000, 15000])
        # labels = (livelli / 1000).astype(int)
        # hex_colors = [mcolors.to_hex(plt.get_cmap('jet')(x)) for x in np.linspace(0, 1, 13)]
        ...
        # ax.set_title(f"ETP {dBZ} dBZ [km]", loc='left')
        ...
        # livelli = np.array([1000, 2000, 3000, 4000, 5000, 6000, 7000, 8000, 9000, 10000, 11000, 12000, 15000])
        # labels = (livelli / 1000).astype(int)
        # colori = [mcolors.to_hex(plt.get_cmap('jet')(x)) for x in np.linspace(0, 1, 13)]
        ...
        # plt.savefig(
        #     "./static/icone/colorbar_radar_etp.png",
        #     dpi=600,
        #     transparent=True,
        #     bbox_inches="tight",
        #     pad_inches=0.1
        ...
        COLORI_HEX = ['#ffffff', 'none'] + [mcolors.to_hex(plt.get_cmap('jet')(x)) for x in np.linspace(0, 1, 13)]
